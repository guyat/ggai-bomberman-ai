...
- CASE13: Tricky / trap-oriented
  - 現時点では `straight_kill` になっている
- CASE14: Tricky trap test
  - 現時点では `straight_kill` になっている
  - ここを今後 `guided_trap_v2` に寄せたい
- CASE15〜17
  - 表示簡略化済み
...
- CASE10〜17 は通っている
- CASE13 は `straight_kill`
- CASE14 は `straight_kill`
- CASE14 を `guided_trap_v2` に寄せたい、というのが次スレの主題
...
CASE14 で
**`straight_kill` ではなく `guided_trap_v2` を優先させること**
...
* CASE14 を `straight_kill` ではなく `guided_trap_v2` に寄せたい
