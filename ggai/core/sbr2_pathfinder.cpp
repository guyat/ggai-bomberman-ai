...
SBR2PathFinder::SBR2PathFinder(const SBR2Board& board, const SBR2Simulator& simulator)
    : board_(board), simulator_(simulator)
{
}
...
bool SBR2PathFinder::can_reach_safe_cell(i8 start_x, i8 start_y, i32 start_frame) const
{
    ...
    while (!q.empty()) {
        ...
        // 今いる場所で少し待てるなら、とりあえず安全地帯に到達できたとみなす
...
) const
{
    ...
    bool visited[512][SBR2Board::HEIGHT][SBR2Board::WIDTH] = {};
    ...
    if (start_frame < 0 || start_frame >= 512) {
        return false;
    }
...
) const
{
    ...
    while (!q.empty()) {
        ...
        if (safe_long_enough) {
            ...
            if (!actions.empty()) {
                out_result.first_action = actions[0];
            } else {
