...
README下部の古いフェーズ表では
「Phase 2: 爆弾設置AI 実装中」
のような表現が残っている箇所があるが、**現状はそれよりかなり進んでいる**。
...
### 次の実装タスク
**「同じ方向に無駄に突っ込みすぎない」処理の実装**

### 目的
- 一本道でずっと同方向に寄り続ける不自然さを減らす
- 少し揺らぎや分岐を入れる
- より人間っぽい再配置にする

### その次
- Aggressive / Careful / Tricky で寄り方を少し変える
...
README下部に大量の planned 技術メモがあるが、これは **消さないこと**。  
今後の動画検証で精密に固めるための重要資産である。
...
**短く削るのではなく、長いREADMEの先頭に「今どこまで来たか」を追記して管理する。**

---

## 次スレでの再開テンプレ（そのまま貼る用）

以下を次スレ冒頭に貼れば、かなり高い確率で続きから再開できる。
...
前提:
- プログラミングはかなり不慣れなので、どのファイルのどこをどう直すか、丸ごと置き換えか一部修正か、コピペしやすい形で丁寧にお願いします。
- README と現在コードを読んだうえで進めてください。
- SBR2 は scp_driver による仮想コントローラー入力前提です。
- READY 中は移動可能、GO! から爆弾設置可能です。
- center → next = 6F、normal move = 11F です。
- 将来的に斜め入力コーナリングを移動モデルの一部として管理予定です。

現在位置:
- 危険回避AI、爆弾設置後の逃走可否判定、直線キル、1歩先予測、trap基礎、Lv1〜20、Aggressive/Careful/Tricky、人間っぽい再配置、壁に向かうだけの移動抑制まで実装済みです。
...
# ============================================
# ここまで README 先頭固定ブロック
# 既存の長い README 本文はこの下に残すこと
# ============================================


``` これより下記は2026/03/18以前のものです。 ```
# GGAI Bomberman AI
...
※各テクニックは動画によるフレーム検証後に実装する。
現時点では実装順のみ定義し、詳細仕様は未確定とする。
